...
##### 本地缓存，远程缓存
* 应用程序 = 算法 + 数据
    * 适合缓存的数据：读写比高的热数据（读写比高，命中率高）。
* 缓存：加速业务处理的临时冗余数据
    * 加载：全量加载（启动慢），懒加载（策略复杂；同步，异步）。
    * 有效性：读写比（写操作导致数据变动），命中率（数据的价值）。
    * 本地缓存
        * in Hibernate：cache 的开启放在 SQL 语句中。
        * in MyBatis：cache 的开启放在对象和表对应的实体类配置中。
        * in Guava：使用类似 map
        * in Spring Cache：基于注解和 AOP，可配置 Condition 和 SPEL 表达式语言。
        * 缺点：多集群，缓存读写放大；占用内存；影响业务，抢占资源。
        * 解决方案：远程缓存。
    * 远程缓存
        * 解决的问题：多集群环境下的同步，占用 JVM 内存，缓存数据的调度。
        * 解决方案一：Redis / Memcached 缓存中间件。Redis 是支持网路、基于内存也支持持久化的日志型、key-value 存储系统。Memcached：开源高性能、分布式内存对象缓存系统。
        * 解决方案二：Hazelcast / Ignite 内存网格，屏蔽网络差异，串起本地缓存和远程缓存。
    * 缓存策略
        * 数据容量：设计容量、使用容量、峰值容量。
        * 策略：过期/清理/淘汰，让更有价值的数据进入缓存。
            * 按 FIFO / LRU
            * 按固定时间
            * 按业务时间
##### Redis 
* 基本功能
    * 基本数据结构：字符串（可用于接收任意格式的数据），散列（Hash；可看作 map 或 POJO 对象），列表（相当于 JAVA 中的 LinkedList），集合（相当于 JAVA 中的 set；无重复元素），有序集合（每个元素关联一个数值 score）。
    * 高级数据结构：Bitmaps 位图，Hyperloglogs，GEO 地理信息。
    * 数据处理：内存数据处理线程一定是单线程！
    * 编程模型：确定性系统（在处理所有数据时必须使用单线程）。
* 6大场景
    * 业务数据缓存
        * 要求：实时热数据
        * 特定：集中式缓存
    ...
    * 全局一致计数
        ...
        * 场景：秒杀，抢红包
    * 高效统计计数
        * 处理：全局 Bitmap 操作（全局 ID 的位操作）
        * 非严格一致性要求的大批量数据，使用 Hyperloglogs 类型。
    * 发布订阅与 Stream
        * 模拟 MQ：Pub-Sub 模拟队列
        * redis 5.0 版本新增数据结构：Redis Stream
    * 分布式锁
        * 出发点：分布式场景全局地锁住某些资源
        * 处理：获取锁（单个原子性操作），释放锁（先判断，有锁再删除）。
* Redis 的 JAVA 客户端
    * Jedis：类似 JDBC，封装了 Redis 命令；驱动包基于 BIO，需要配置连接池。
    * Lettuce：驱动包基于 Netty NIO，线程安全。
    * Redission：Redis 的 JAVA 分布式组件库，驱动包基于 Netty NIO，线程安全；实现了大量分布式的功能特性。
* Redis 与 Spring 整合
    * 组件：Spring Data Redis
        * 核心：RedisTemplate（可配置为基于 Jedis/Lettuce/Redission）
    * Spring Boot 与 Redis 
        * 引入 spring-boot-starter-data-redis
    * Spring Cache 与 Redis 
        * 引入 spring-boot-starter-cache
        * 默认使用全局的缓存管理器 CacheManager，实现自动集成。
        * 使用 Redis 缓存时，默认用 JAVA 的对象序列化（对象需要实现 Serializable 接口；序列化方式可自定义）。
        * 使用 ConcurrentHashMap / Ehcache 缓存时，本地缓存，无需考虑序列化。
    * MyBatis 项目集成 cache 
        * 目标：增加缓存机制，加速 ORM 数据库读操作。
        * 【数据库层】Spring Boot 集成 MyBatis，对外配置 REST 接口。
        * 【ORM 层】本地 Ehcache 与 MyBatis 集成，实现 MyBatis 的二级缓存。
        * 【Service 层】本地 Ehcache 与 Spring cache 集成，实现 Service 方法级别（mapper）缓存。
        * 【配置】本地缓存 Ehcache 替换为 Redis 远程缓存；默认的 JAVA 对象序列化方式替换为 jackson json 序列化。
* 高级功能
    * Redis 事务：Redis 的单线程模型可以很方便地实现事务机制。
        * 操作：开启，提交，回滚。
        * 优化：实现乐观锁，watch 某个 key，发现有变化则事务失败。
    * Redis Lua
        * Redis server 内置 Lua 脚本引擎。
        * 预编译：加速脚本访问速度。
        * 单线程：每个脚本的执行不会被其他线程打断，保证事务性。
    * Redis 管道技术
        * 一系列不相关的命令交由 Redis Server 执行，批处理，结果批量返回。
    * Redis 数据恢复与备份
        * （默认备份）RDB：全量备份数据，生成“.rdb 文件”。恢复时将改文件放在“/data”路径下，重启 redis 时初始化数据。
        * 追加日志文件（AOF）：所有操作以命令的方式记录在 AOF 文件（“.aof 文件”）中。恢复时，AOF 文件可以在 redis 启动时自动加载。
    * 性能优化
        * 内存：优化内部表示，提升内存使用效率。
        * CPU：永远不要阻塞单线程 CPU！
    * 分区
        * 解决内存容量问题。
        * 类比数据库的垂直拆分。做好 key 的规划，尽量分区，避免多个业务系统相互干扰。
* Redis 集群与高可用
    * Redis 主从复制
        * 发展：Redis 内存数据库从单节点到多节点
        * 主从复制，读写分离：主库处理写请求；从库只读，异步复制主库（也可以从其他从库复制数据），处理读请求。
    * Redis Sentinel 主从切换：走向 MHA
        * Sentinel 监控主、从节点的在线状态，负责执行切换；无需配置从节点。
        * Sentinel 也可以做成集群。
    * Redis Cluster：走向分片，全自动分库分表
        * 一致性哈希方式
        * 解决容量问题：数据分散到多个服务节点（数据槽位通过哈希算法得到）。
        * 一致性要求：默认所有槽位可用时才对外提供服务。
        * 配合主从模式使用。
* Hazelcast 内存网格
    * 特性：分布式（数据分散），高可用（多副本），可扩展，面向对象（OO 和非关系型数据模型），低延迟（基于内存，可用堆外内存）。
    * 架构
        * 支持：事务、各种语言、各种缓存技术；内置分片、集群功能；内部基于分布式协议确保一致性。
    * 部署模式
        * client-server 模式：Hazelcast 多节点组成集群，应用服务的集群访问 Hazelcast 集群。
        * Embedded 模式：Jar 包作为依赖，直接放入应用程序，在 JVM 上做本地缓存。
    * 数据分区
        * 有默认分区个数（可配）；所有分区均匀；支持多副本机制，高可用。
    * 集群与高可用
        * 默认 AP，集群都是自动化管理。
        * 自动计算分区平衡，业务无感知。
        * 问题：auto rebalance 产生性能抖动；频繁重启，可能导致线上脑裂。
    * 事务支持
        * 支持 ONE_PHASE：节点宕机时可能导致系统不一致。
        * 支持 TWO_PHASE：提交前增加一个 PREPARE 阶段，检查冲突；若本节点宕机，备份节点根据 commit log 完成事务提交。
    * 数据亲密性    
        * 场景：因为数据分布在网格的不同节点，当前操作所属节点和被操作的数据所属节点不同，导致频繁的数据传输。

#### 经验认识
* 缓存常见问题
    * 缓存穿透：大量并发、查询不存在的 key。
        * 解决：缓存空值的 key；过滤，判断 key 是否存在；完全以缓存为准，即刻返回结果或异步延迟加载。
    * 缓存击穿：某个具体的 key 失效时，正好有大量并发请求访问。
        * 解决：key 的更新增加全局互斥锁；完全以缓存为准，即刻返回结果或异步延迟加载。
    * 缓存雪崩：某一时刻发生大规模的缓存失效`时，会有大量请求直接打到数据库。
        * 解决：更新时间尽量均匀；热数据尽量分散；主从复制/多副本，实现高可用；实现熔断限流。
* Redis 使用经验    
    * 性能
        * client 端线程数和连接数不宜过大
        * 评估当前缓存的性价比：（指标）操作读写比，命中率。
    * 容量
        * 确保缓存资源充分利用
        * 监控，注意增量变化（增量大时影响缓存使用效率）
    * 资源管理和分配
        * 尽量使用独立的 Redis
        * 控制资源申用，规范环境和 Key 的管理（防止冲突、数据错乱）
